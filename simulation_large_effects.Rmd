...
tau[(sum(choose(K,0:0))+1):sum(choose(K,0:1))] = 1.00 * sign(runif(choose(K,1),-1, 1))
tau[(sum(choose(K,0:1))+1):sum(choose(K,0:2))] = 0.50 * sign(runif(choose(K,2),-1, 1))
...
simulation.res.strong <- comb.select.trial(factorial.data.init, 
                                        ...
                                        Niter = 300, num.ties = 1)
# saveRDS(simulation.res.strong, "simulation_large_effects.rds")
...
```{r}
simulation.res.strong = readRDS("simulation_large_effects.rds")
```
