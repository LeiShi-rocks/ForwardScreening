...
effect_size = c(0.12, 0.14, 0.16, 0.20, 0.30, 0.50)
for (size_ind in 1:6){
  K = 8
  num_trt_group = 2^K
...
```{r}
effect_size = c(0.10, 0.12, 0.14, 0.20, 0.30, 0.50)
model_report = list()
...
model_report_df = data.frame(
  selection = c(model_report[[1]][,1], model_report[[2]][,1], model_report[[3]][,1], 
                model_report[[4]][,1], model_report[[5]][,1], model_report[[6]][,1])/500, 
  size = c(rep(effect_size[1], 8), rep(effect_size[2], 8), rep(effect_size[3], 8), 
           rep(effect_size[4], 8), rep(effect_size[5], 8), rep(effect_size[6], 8)), 
  Method = c(rep(c("FB", "WB", "Forward Bonferroni", "FL", "WL", 
                   "Forward Lasso", "Naive Bonferroni", "Naive Lasso"), 6))
)
...
rejection_report_df = data.frame(
  ...
  size = c(rep(effect_size[1], 9), rep(effect_size[2], 9), rep(effect_size[3], 9), 
           rep(effect_size[4], 9), rep(effect_size[5], 9), rep(effect_size[6], 9)), 
  Method = c(rep(c("FB", "WB", "Forward Bonferroni", "FL", "WL", 
                   "Forward Lasso", "Naive Bonferroni", "Naive Lasso", "No Selection"), 6))
)

length_report_df = data.frame(
  CI_length = c(coverage_report[[1]][,2], coverage_report[[2]][,2], coverage_report[[3]][,2], 
                coverage_report[[4]][,2], coverage_report[[5]][,2], coverage_report[[6]][,2])*2,
  size = c(rep(effect_size[1], 9), rep(effect_size[2], 9), rep(effect_size[3], 9), 
           rep(effect_size[4], 9), rep(effect_size[5], 9), rep(effect_size[6], 9)), 
  Method = c(rep(c("FB", "WB", "Forward Bonferroni", "FL", "WL", 
                   "Forward Lasso", "Naive Bonferroni", "Naive Lasso", "No Selection"), 6))
...
combined_report$size = combined_report$size/0.50 * true_effect
```
...
plot_coverage = ggplot(combined_report %>% filter(!Method %in% c("FB", "FL", "WB", "WL", "No Selection")), 
  ...
  theme_classic(base_size = 20) +
  scale_x_continuous(limits = c(0.26, 1.25))
